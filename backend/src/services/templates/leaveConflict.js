function leaveConflictTemplate(payload) {
  ...
  const formattedStart = startsAt ? new Date(startsAt).toLocaleString() : date;
  ...
  return {
    ...
    html: `
      <div style="font-family: sans-serif; background: #080c14; color: #f1f5f9; padding: 32px; border-radius: 16px;">
        ...
        <p>Dr. ${doctorName} is unavailable on <strong>${formattedStart}</strong> due to scheduled leave.</p>
